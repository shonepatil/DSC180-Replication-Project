...
def train_test(A, X, y, idx_train, idx_val, idx_test, 
    ...
    def train(epoch):
        ...
        optimizer.zero_grad()
        output = model(X, A)
        loss_train = F.binary_cross_entropy(output[idx_train].cuda(), y[idx_train].cuda())
        ...
        loss_val = F.binary_cross_entropy(output[idx_val].cuda(), y[idx_val].cuda())
    ...
    def test():
        model.eval()
        output = model(X, A)
        loss_test = F.binary_cross_entropy(output[idx_test].cuda(), y[idx_test].cuda())
